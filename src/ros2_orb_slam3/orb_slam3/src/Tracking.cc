...
static std::ofstream g_csvLogFile;
static long g_csvCurrentSecond = -1;
static double g_start_time = -1.0;
static double last_log_time = -1.0;
static std::string g_csvLogsFolder = "logs";

// helper: create directory if not exists (POSIX)
static inline void ensure_logs_folder_exists(const std::string &folder)
{
    struct stat st;
...
namespace ORB_SLAM3
{
// --- add this inside namespace ORB_SLAM3 { ... } before constructor definition ---


static void LogFrameCSV(double timestamp,
                        ...
                        int tracking_state)
{
    ...
    if(!g_csvLogFile.is_open())
    {
        ensure_logs_folder_exists(g_csvLogsFolder);

        std::string filename = g_csvLogsFolder + "/tracking_log.csv";
        g_csvLogFile.open(filename, std::ios::out | std::ios::trunc);

        if(g_csvLogFile.is_open())
        {
            g_csvLogFile << "datetime,timestamp,tracking_time_ms,fps,keypoints,map_points,"
                         << "x,y,z,qx,qy,qz,qw,tracking_state\n";
        }
        else
        {
            std::cerr << "ERROR opening CSV log file: " << filename << std::endl;
            return;
        }
    }
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    // 1. START TOTAL TIMER HERE (Before Frame Creation)
    ...
    // This part creates the frame and does ORB extraction
    ...
    // This performs Motion Model, Search Local Map, and Pose Optimization
    Track();

    // 2. STOP TOTAL TIMER HERE (After all tracking is done)
    auto t_total_end = std::chrono::steady_clock::now();
    double total_tracking_time_ms = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t_total_end - t_total_start).count();

    // 3. LOG TO CSV AND STATISTICS
#ifdef REGISTER_TIMES
    // Record the actual total tracking time into your stats vector
    vdTrackTotal_ms.push_back(total_tracking_time_ms);
    
    // We still keep sub-times for deeper analysis
    vdORBExtract_ms.push_back(mCurrentFrame.mTimeORB_Ext);
    vdStereoMatch_ms.push_back(mCurrentFrame.mTimeStereoMatch);
#endif

    if(g_start_time < 0) g_start_time = mCurrentFrame.mTimeStamp;
    double relative_time = mCurrentFrame.mTimeStamp - g_start_time;
    double fps = 1000.0 / total_tracking_time_ms;

    if(last_log_time < 0 || (relative_time - last_log_time) >= 1.0) {
        last_log_time = relative_time;
        // Now logging the TRUE total tracking time
        LogFrameCSV(relative_time, total_tracking_time_ms, fps, mCurrentFrame.N, mnMatchesInliers, mCurrentFrame.GetPose(), mState);
    }
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB,const cv::Mat &imD, const double &timestamp, string filename)
{
  ...
  if(g_start_time < 0)
  {
      g_start_time = mCurrentFrame.mTimeStamp;
  }
  ...
  double tracking_time_ms = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(t_end - t_start).count();
  double fps = 1000.0 / tracking_time_ms;
  std::cout << "Tracking FPS: " << fps << std::endl;
  // keypoints: gunakan mCurrentFrame.N
  // map_points: gunakan mnMatchesInliers (nilai dijaga di dalam track setelah TrackLocalMap)
  int keypoints = mCurrentFrame.N;
  int map_points = mnMatchesInliers;
